...
const SYSTEM_BY_TYPE: Record<LetterType, string> = {
...
export async function POST(request: NextRequest) {
  ...
  try {
    const msg = await anthropic.messages.create({
      ...
      max_tokens: 320,
      system: SYSTEM_BY_TYPE[letterType],
      messages: [{
        role: 'user',
        content: `Property details: ${featuresText}. Address: ${address.trim()}.${name ? ` Homeowner: ${name}.` : ''} Write the letter body now.`,
      }],
    })
    const block = msg.content[0]
    if (block.type === 'text') aiCopy = block.text.trim()
  } catch (err) {
    console.error('[letters/analyze] AI copy failed:', err)
    aiCopy = `We provide professional lawn care across Kendallville and the surrounding area, and we'd love to take care of your lawn this season.\n\nWe keep things simple — no contracts, no hassle, just a sharp, well-kept lawn you can be proud of. A personalized estimate for your property is included below.`
  }
