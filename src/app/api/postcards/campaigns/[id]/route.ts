...
export async function GET(
  ...
  const { data: campaign, error: campaignError } = await (adminClient.from('postcard_campaigns') as any)
    .select('*')
    .eq('id', id)
    .single()
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const { data: recipients, error: recipientsError } = await (adminClient.from('postcard_recipients') as any)
    .select('*')
    .eq('campaign_id', id)
