...
import {
  FileText, RefreshCw, ChevronDown, ChevronUp, CheckCircle2,
  XCircle, Clock, Loader2, Send, Eye, History, Settings2,
  ArrowLeft, Plus, Trash2, Zap, Database, RotateCcw,
} from 'lucide-react'
...
type LetterType = 'general' | 'new_homeowner' | 'violation'
...
export default function LettersPage() {
  ...
  function addFromDb() {
    ...
    for (const id of dbSelected) {
      ...
      if (lead && lead.address) {
        ...
        toAdd.push({
          id: crypto.randomUUID(), name: lead.name,
          address: parts[0] ?? lead.address, city: parts[1] ?? '',
          state: parts[2]?.split(' ')[0] ?? 'IN', zip: parts[2]?.split(' ')[1] ?? '',
          status: 'pending',
        })
      } else if (cust && cust.address) {
        const parts = cust.address.split(',').map(s => s.trim())
        toAdd.push({
          id: crypto.randomUUID(), name: cust.full_name,
          address: parts[0] ?? cust.address, city: parts[1] ?? '',
          state: parts[2]?.split(' ')[0] ?? 'IN', zip: parts[2]?.split(' ')[1] ?? '',
          status: 'pending',
        })
      }
  ...
  async function analyzeRow(id: string) {
    ...
    try {
      ...
      const res = await fetch('/api/letters/analyze', {
        ...
        body: JSON.stringify({ address: fullAddress, name: row.name, letter_type: campaign.letter_type }),
      })
      ...
      setAddresses(prev => prev.map(r =>
        r.id === id
          ? {
              ...r, status: 'ready',
              ai_copy: data.ai_copy, quote_amount: data.quote_amount,
              lot_size: data.lot_size, sq_footage: data.sq_footage,
            }
          : r
      ))
    } catch (err) {
  ...
  async function launchCampaign() {
    ...
    setLaunching(true)
    try {
      const campaignRes = await fetch('/api/letters/campaigns', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          name: campaign.name,
      ...
      const sendRes = await fetch('/api/letters/send', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          ...
          recipients: readyRows.map(r => ({
            ...
            state: r.state, zip: r.zip, ai_copy: r.ai_copy ?? '',
            quote_amount: r.quote_amount ?? 35, lot_size: r.lot_size, sq_footage: r.sq_footage,
          })),
        }),
      })
      ...
      if (!sendRes.ok) {
        ...
        throw new Error(d.error ?? 'Failed to send letters')
      }
  ...
  async function saveResponseRate(campaignId: string) {
    ...
    const res = await fetch(`/api/letters/campaigns/${campaignId}`, {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
  ...
  async function resendCampaign(c: Campaign) {
    ...
    try {
      const res = await fetch(`/api/letters/campaigns/${c.id}`)
      ...
      if (!recipients.length) { toast.error('No recipients found for this campaign'); return }

      const newCampaignRes = await fetch('/api/letters/campaigns', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
      ...
      const sendRes = await fetch('/api/letters/send', {
        ...
        body: JSON.stringify({
          ...
          recipients: recipients.map((r) => ({
            ...
            name: r.name ?? 'Homeowner', address: r.address ?? '',
            city: r.city ?? '', state: r.state ?? 'IN', zip: r.zip ?? '',
            ai_copy: r.ai_copy ?? '', quote_amount: r.quote_amount ?? 35,
            lot_size: r.lot_size, sq_footage: r.sq_footage, source: r.source,
          })),
        }),
      })
  ...
  async function loadMockup() {
    ...
    try {
      const res = await fetch(`/api/letters/preview?${params}`)
      ...
      <div className="flex flex-wrap items-center justify-between gap-y-2 mb-6">
        <div>
          ...
          <p className="text-sm text-zinc-500 mt-1">
            Send personalized lawn-care letters to targeted addresses via Lob
          </p>
        </div>
        <div className="flex gap-2">
          <Button variant={activeView === 'setup' ? 'default' : 'outline'} size="sm" onClick={() => setActiveView('setup')} className="gap-1.5">
            <Settings2 size={14} /> Setup
          </Button>
          <Button variant={activeView === 'preview' ? 'default' : 'outline'} size="sm" onClick={() => { setActiveView('preview'); loadMockup() }} className="gap-1.5">
            <Eye size={14} /> Preview Letter
          </Button>
          <Button variant={activeView === 'history' ? 'default' : 'outline'} size="sm" onClick={() => { setActiveView('history'); loadHistory() }} className="gap-1.5">
      ...
      <div className="bg-white rounded-xl border border-zinc-200 p-4 mb-4">
        <div className="flex flex-wrap items-center justify-between gap-2 mb-2">
          <div className="text-sm font-medium text-zinc-700">
            Budget: {addresses.length} letters × {formatCurrency(PRICE_PER_PIECE)} ={' '}
        ...
        <div className="space-y-4">
              ...
              {inputTab === 'paste' ? (
                ...
                <div className="space-y-3">
                  {dbLoading ? (
                    ...
                    <>
                      <p className="text-xs text-zinc-500">Select leads and customers to add to your campaign.</p>
          ...
          {addresses.length > 0 && (
            <div className="bg-white rounded-xl border border-zinc-200 overflow-hidden">
              <div className="flex flex-wrap items-center justify-between gap-2 px-4 py-3 border-b border-zinc-100">
                ...
                <Button size="sm" onClick={analyzeAll} disabled={analyzing || pendingCount === 0} className="gap-1.5">
                  {analyzing
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="bg-zinc-50 text-left">
                      ...
                      <th className="px-4 py-2.5 font-medium text-zinc-500 text-xs uppercase tracking-wide">Letter Copy</th>
            ...
            {configOpen && (
              <div className="px-4 pb-4 grid grid-cols-1 sm:grid-cols-2 gap-4 border-t border-zinc-100">
                <div className="space-y-1.5">
                  ...
                  <Input placeholder="Spring 2026 Letters" value={campaign.name} onChange={e => setCampaign(p => ({ ...p, name: e.target.value }))} />
                </div>
                <div className="space-y-1.5">
                ...
                <div className="space-y-1.5">
                  ...
                  <Input placeholder="Optional description" value={campaign.description} onChange={e => setCampaign(p => ({ ...p, description: e.target.value }))} />
                </div>
                <div className="space-y-1.5">
                  <Label>Tags (comma-separated)</Label>
                  <Input placeholder="spring, kendallville, test" value={campaign.tags} onChange={e => setCampaign(p => ({ ...p, tags: e.target.value }))} />
                </div>
                <div className="space-y-1.5">
                  <Label>Send Date</Label>
                  <Input placeholder="ASAP" value={campaign.send_date} onChange={e => setCampaign(p => ({ ...p, send_date: e.target.value }))} />
                </div>
                <div className="space-y-1.5">
                  <Label>Budget Cap ($)</Label>
                  <Input type="number" min={0} step={10} value={campaign.budget_cap} onChange={e => setCampaign(p => ({ ...p, budget_cap: parseFloat(e.target.value) || 0 }))} />
                </div>
              </div>
            )}
          </div>
          ...
          <div className="space-y-3">
            <div className="flex flex-wrap items-center gap-3">
              <Button size="lg" onClick={launchCampaign} disabled={!canLaunch} className="gap-2">
                {launching
                  ? <><Loader2 size={16} className="animate-spin" /> Launching…</>
                  : <><Send size={16} /> Launch Campaign</>}
              </Button>
              {!campaign.name.trim() && <p className="text-xs text-amber-600">Enter a campaign name to launch</p>}
              {pendingCount > 0 && !analyzing && <p className="text-xs text-amber-600">{pendingCount} address{pendingCount !== 1 ? 'es' : ''} still need analysis</p>}
              {missingZipCount > 0 && <p className="text-xs text-red-600">{missingZipCount} address{missingZipCount !== 1 ? 'es are' : ' is'} missing a ZIP code — Lob requires it</p>}
              {overBudget && <p className="text-xs text-red-600">Over budget — cannot launch</p>}
            </div>
            ...
            {launchError && (
              <div className="bg-red-50 border border-red-200 rounded-xl p-4">
                <div className="flex items-start justify-between gap-3">
                  ...
                  <button onClick={() => setLaunchError(null)} className="text-red-400 hover:text-red-600 flex-shrink-0 text-lg leading-none">×</button>
                </div>
              </div>
            )}
          </div>
      ...
      {activeView === 'preview' && (
        <div className="space-y-4">
          ...
          <div className="bg-zinc-100 rounded-xl p-6 flex flex-col items-center gap-2">
            <p className="text-xs font-medium text-zinc-500 uppercase tracking-wide mb-2">8.5×11 Letter</p>

            {mockupLoading ? (
              <div className="flex items-center justify-center" style={{ width: '612px', height: '792px' }}>
                <Loader2 size={32} className="animate-spin text-zinc-400" />
              </div>
            ) : mockupHtml ? (
              /* 8.5in×11in at 96dpi = 816×1056px, scaled to 612px wide (0.75) */
              <div
                style={{
                  width: '612px', height: '792px', overflow: 'hidden',
                  borderRadius: '4px', boxShadow: '0 8px 32px rgba(0,0,0,0.18)',
              ...
              >
                <iframe
                  srcDoc={mockupHtml}
                  style={{
                    width: '816px', height: '1056px', border: 'none',
                    transformOrigin: 'top left', transform: 'scale(0.75)',
                  }}
              ...
              <div className="flex items-center justify-center text-zinc-400 text-sm" style={{ width: '612px', height: '792px' }}>
                Click &quot;Regenerate&quot; to load the preview
              </div>
            )}

            <p className="text-xs text-zinc-400 mt-1">
              The blank top area is reserved for the mailing address (shows through a windowed envelope)
            </p>
          </div>
        </div>
      )}
                    ...
                    <tr key={c.id} className="hover:bg-zinc-50">
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-2">
                          ...
                          />
                          <Button size="sm" variant="outline" className="h-7 px-2 text-xs" onClick={() => saveResponseRate(c.id)}>
                            Save
                          </Button>
                        </div>
                      </td>
                      <td className="px-4 py-3">
                        <Button size="sm" variant="outline" className="gap-1.5 whitespace-nowrap" disabled={!!resending[c.id]} onClick={() => resendCampaign(c)}>
                          {resending[c.id]
  ...
  return (
    <tr className="hover:bg-zinc-50">
      ...
      <td className="px-4 py-3 text-zinc-500 text-xs max-w-[200px]">
        ...
        {!row.zip?.trim() && <span className="text-red-500 font-medium">⚠ Missing ZIP</span>}
      </td>
      ...
      <td className="px-4 py-3 max-w-[260px]">
        {row.ai_copy ? (
          <div>
            <p className={`text-xs text-zinc-600 leading-relaxed whitespace-pre-wrap ${expanded ? '' : 'line-clamp-2'}`}>
              {row.ai_copy}
            </p>
            <button onClick={() => setExpanded(p => !p)} className="text-xs text-purple-500 hover:text-purple-700 mt-0.5">
              {expanded ? 'Less' : 'More'}
            </button>
          </div>
        ) : (
      ...
      <td className="px-4 py-3">
        <div className="flex items-center gap-1.5">
          {(row.status === 'pending' || row.status === 'failed') && (
            <Button size="sm" variant="outline" className="h-7 px-2 text-xs gap-1" onClick={onAnalyze}>
              <Zap size={11} /> Analyze
            </Button>
          )}
          <Button size="sm" variant="ghost" className="h-7 px-2 text-zinc-400 hover:text-red-500" onClick={onRemove}>
            <Trash2 size={13} />
          </Button>
        </div>
      </td>
